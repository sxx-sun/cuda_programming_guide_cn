...
- 组内各线程在同步点之前完成的所有内存操作（包括读、写），在同步点之后的对组内所有线程可见。
- 组内线程全部抵达同步点后，才允许线程执行同步点之后的代码。
...
- ``barrier_wait`` 仅保证组内所有线程均已调用 ``barrier_arrive`` ，并不保证所有线程都执行到了 ``barrier_wait`` [#]_。

.. [#] ``barrier_wait`` 是一个阻塞动作，放行的条件是所有的线程都执行了 ``barrier_arrive`` 。
      所以 ``barrier_wait`` 能继续往下执行， 说明所有的线程已经执行了到达，但是否所有线程都执行了 ``barrier_wait`` ， 这个是不保证的。
...
这对优化数据传输、让计算与数据搬运并行以提升性能方面很有用。

``memcpy_async`` 函数用于启动一个从全局内存到共享内存的异步搬运操作。
其设计初衷是提供类似 `预取（prefetch）` 的机制，使在数据在真正使用之前，提前加载好。

``wait`` 函数会强制线程组内所有线程等待，直至异步内存加载完成。
...
仅当源地址为全局内存、目标地址为共享内存，且二者均至少是 4 字节对齐时， ``memcpy_async`` 才能异步执行。
...
前文介绍的所有协作组功能均可在这类大线程组上使用，但有一处需要重点注意：若要对整个网格执行同步，必须使用接口 ``cudaLaunchCooperativeKernel`` 来启动核函数。
...
例如下面的示例， 代码中如果设备 `0` 支持该属性， ``cudaDeviceGetAttribute`` 会将 ``supportsCoopLaunch`` 设置为 ``1`` 。
