...
如果 ``result`` 不为空（即非 dry-run），则 ``result`` 中对应的组也会将其 ``smCount`` 设置为相同的值。
``nbGroups`` 个 ``groupParams`` 条目的指定顺序至关重要，因为它们会从左（索引 0）到右（索引 ``nbGroups-1`` ）依次进行评估。

:numref:`table-split-api-overview` 提供了 ``cudaDevSmResourceSplit`` 接口所支持参数的概览。
...
.. list-table:: ``cudaDevSmResourceSplit`` 切分接口概览
   ...
   * - ``result``
     - 存放切分结果的数组
     - 探索性 dry-run 时为 ``nullptr`` ；否则为非空指针
...
1. ``cudaDevSmResourceSplit`` 接口的返回值取决于 ``result`` ：

   - ``result != nullptr`` ：仅当切分成功且创建了 ``nbGroups`` 个满足指定要求的有效分组时，该接口才返回 ``cudaSuccess`` ；否则返回错误。
     由于不同类型的错误可能返回相同的错误码（例如 ``CUDA_ERROR_INVALID_RESOURCE_CONFIGURATION`` ），建议在开发期间使用 ``CUDA_LOG_FILE`` 环境变量以获得更具体的错误信息。

   - ``result == nullptr`` ： `dry-run` 模式。 即使某个组的最终 ``smCount`` 为零，该接口也可能返回 ``cudaSuccess`` ；而在 ``result`` 非空的情况下，这种情况本应返回错误。
     可将此模式视为一种测试，可用于探索支持的功能，尤其是在发现模式中。

2. 当 ``result != nullptr`` 且调用成功时，生成的 ``result[i]`` 的资源类型为 ``cudaDevResourceTypeSm`` ，
...
3. 如果 ``coscheduledSmCount`` 和 ``preferredCoscheduledSmCount`` 中的任意一个指定零，表示使用这些字段的默认值。
   这些默认值因 GPU 而异，都等于通过 ``cudaDeviceGetDevResource`` 接口检索指定设备得到的 SM 资源的 ``smCoscheduledAlignment`` 。
...
.. code-block:: c++
  ...
  CUDA_CHECK(cudaDeviceGetDevResource(gpu_device_index,
                                      &initial_GPU_SM_resources,
                                      cudaDevResourceTypeSm));
  std::cout << "Default value will be equal to "
            << initial_GPU_SM_resources.sm.smCoscheduledAlignment
            << std::endl;
  ...
  cudaDevSmResourceGroupParams group_params_tmp = {.smCount=0,
                                                   ...
                                                   .flags=0};
  CUDA_CHECK(cudaDevSmResourceSplit(nullptr,
                                    1,
                                    &initial_GPU_SM_resources,
                                    nullptr /*remainder*/,
                                    default_split_flags,
                                    &group_params_tmp));
  std::cout << "coscheduledSmcount default value: "
            << group_params.coscheduledSmCount
            << std::endl;
  std::cout << "preferredCoscheduledSmcount default value: "
            << group_params.preferredCoscheduledSmCount
            << std::endl;

4. 如果存在剩余组，则其 SM 数量或协同调度要求没有任何约束。这需要由用户自行探索。

在详细介绍 ``cudaDevSmResourceGroupParams`` 结构体各个字段之前， 我们通过一些示例用例展示了这些值可能的情况。
假设 ``initial_GPU_SM_resources`` 已经按前面示例的方式进行了填充，且这就是将要被切分的资源。
表中的每一行都以此相同的起点开始。
...
.. code-block:: c++
   ...
   // Update depending on number of groups planned.
   // Increase size if you plan to also use a workqueue resource
   ...
   CUDA_CHECK(cudaDevSmResourceSplit(&result_use_case[0],
                                     nbGroups,
                                     &initial_GPU_SM_resources,
                                     remainder,
                                     default_split_flags,
...
- 用例1: 划分一个具有 16 个 SM 的分组，不关心剩余分组，可能使用簇。

+----------+------------+----+---------------------------------------------------------------------+
|          |            |    | groupParams[i] fields                                               |
+----------+------------+----+----------+-------------------+------------------------------+-------+
| nbGroups | remainder  | i  | smCount  |coscheduledSmCount | preferredCoscheduledSmCount  | flags |
+==========+============+====+==========+===================+==============================+=======+
|  1       | nullptr    |    |  16      |  0                |  0                           | 0     |
+----------+------------+----+----------+-------------------+------------------------------+-------+

- 用例2a： 划分两个分组，其中一个具有 16 个 SM 的分组，另外一个是剩余分组，不使用簇。
...
| nbGroups | remainder  | i  | smCount  |coscheduledSmCount | preferredCoscheduledSmCount  | flags |
+==========+============+====+==========+===================+==============================+=======+
|  1       |not nullptr |    |  16      |  2                |  2                           | 0     |
+----------+------------+----+----------+-------------------+------------------------------+-------+

- 用例2b： 划分两个分组，其中一个具有 16 个 SM 的分组，另外一个具有其余所有 SM 的资源，无剩余分组，不使用簇。
...
- 用例3: 划分两个分组，分别具有 28 和 32 个 SM，将使用大小为 4 的簇。
...
- 用例4: 划分两个分组，其中一个拥有尽可能多的 SM，并使用大小为 8 的簇，另外一个为剩余分组
...
| nbGroups | remainder  | i  | smCount  |coscheduledSmCount | preferredCoscheduledSmCount  | flags |
+==========+============+====+==========+===================+==============================+=======+
|  1       |not nullptr |    |  0       |  8                |  8                           | 0     |
+----------+------------+----+----------+-------------------+------------------------------+-------+

- 用例5: 划分两个分组，一个具有尽可能多 SM 的资源，能运行大小为 4 的簇，另外一个具有 8 个 SM 的资源。
...
注意：这里的顺序至关重要！更改 ``groupParams`` 数组中条目的顺序，可能会导致没有足够的 SM 留给 8-SM 组。
...
- 有效的非零 ``smCount`` 值要求：
  ``(multiple of 2) and in [2, input->sm.smCount] and ((flags == 0) ? multiple of actual coscheduledSmCount : greater than or equal to coscheduledSmCount)``
...
分割 SM 资源后（以及可选地添加工作队列配置资源），下一步是使用 ``cudaDevResourceGenerateDesc`` 接口为预计可供 GC 使用的所有资源生成资源描述符。
接口如下：
...
鉴于 ``green_ctx`` 是一个 GC ，上述流创建接口的默认流创建标志等效于 ``cudaStreamNonBlocking`` 。

.. _green-contexts-cuda-graphs:

**CUDA 图**

对于作为 CUDA 图（参见 :ref:`cuda-graphs` ）一部分启动的 kernel，还有一些更细微的注意事项。
与 kernel 不同，启动 CUDA 图执行的流并不决定所图执行时使用的 SM 资源，因为该流仅用于依赖关系跟踪。

执行 kernel 节点（以及其他适用的节点类型）使用的 EC 是在节点创建期间设置的。
...
注意，在默认的图追踪模式下，整个图将显示在启动它的流的 GC 之下，但正如前文所解释的，这并不能提供关于各个图节点执行上下文的任何信息。
...
.. code-block:: c++
   ...
   // Assume cudaStream_t gc_stream has already been created
   // and a __global__ void cluster_kernel exists.
   ...
   config.stream=gc_stream; // pass the CUDA stream that will be used for that kernel
   ...
   // Doing so would result in a successful cudaLaunchKernelEx call
   // for the same kernel and launch config.
...
本节介绍一些额外的 GC 接口。如需完整列表，请参阅相关的 CUDA Runtime API 文档 `Execution Context Management <https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__EXECUTION__CONTEXT.html>`_ 。
...
``cudaExecutionCtxRecordEvent`` 记录一个 CUDA 事件， `capturing` 调用时指定执行上下文的中的所有工作 / 活动；
而 ``cudaExecutionCtxWaitEvent`` 使提交给该执行上下文的所有未来工作等待指定事件（之前的所有工作完成）。

.. note::

   英文原文如下：

   cudaExecutionCtxRecordEvent records a CUDA event capturing all work/activities of the specified execution context at the time of this call,
   while cudaExecutionCtxWaitEvent makes all future work submitted to the execution context wait for the work captured in the specified event.

   这个我不会翻译，自己理解吧。

如果执行上下文有多个 CUDA 流，使用 ``cudaExecutionCtxRecordEvent`` 比使用 ``cudaEventRecord`` 更方便。
如果不用此接口，要达到同样效果，需要在该执行上下文的每个流上通过 ``cudaEventRecord`` 分别记录一个 CUDA 事件，然后让依赖工作分别等待所有这些事件。
类似地，如果需要执行上下文的所有流等待一个事件完成，使用 ``cudaExecutionCtxWaitEvent`` 比使用 ``cudaStreamWaitEvent`` 更方便。
替代方案是该执行上下文中的每个流分别调用 ``cudaStreamWaitEvent`` 。

CPU 端的阻塞同步，使用 ``cudaError_t cudaExecutionCtxSynchronize(cudaExecutionContext_t ctx)``  接口。
该调用将被阻塞，直到指定的执行上下文完成其所有工作。
...
从 CPU 端看到的时间线如下：首先在一个长时间运行的 kernel 在整块 GPU 上需要执行多波次 （ ``delay_kernel_us`` ）， 在 CUDA 流 ``strm1`` 先上启动。
然后经过短暂的等待时间（短于长 kernel 的运行时长）之后，一个较短但关键的 kernel （ ``critical_kernel`` ）在流 ``strm2`` 上启动。
两个 kernel 的 GPU 运行时长以及从 CPU 启动到完成的时间都会被测量。

作为长时间运行 kernel 的模拟，使用了一个延迟 kernel ，其中每个线程块运行固定的微秒数，并且线程块数量超过 GPU 可用的 SM 数量。
