...
应用程序若要使用多个 GPU，首先要枚举可用的 GPU 设备；根据硬件属性、CPU 亲和性以及节点间的互联情况，从可用设备中选择，并为即将使用的每个设备创建 CUDA 上下文。
...
设备内存分配和 kernel 启动均会在 `当前设备` 上执行；同时，创建流和事件也会与 `当前设备` 关联。在主机线程调用 ``cudaSetDevice()`` 之前，默认使用 0 号设备。
...
与流的常规行为一致，两个设备内存之间的异步拷贝可以与其他流中的拷贝或 kernel 并发执行。
